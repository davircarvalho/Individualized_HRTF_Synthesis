...
% DAVI ROCHA CARVALHO; ENG. ACUSTICA - UFSM; JUNHO/2020
...
function model_engine(x1, x3,... % anthro cabeca
                      ...
                      table_anthro,... % tabela antropometria multi_indiviiduos
                      output_path)   % diretorio pra salvar
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Load networks
    tic
    ...
    for subj = 1:no_subject 
        if subj > 1
            close(wait)
        end
        wait = waitbar(0,'Neural net inference ..', 'Name', ['Subject: ' num2str(subj)]);
        cont = 0;
        
        % unir as duas matrizes
        InptMtx(:,:,1) = abs([x(subj,:), dL(subj,:)]');  % L 
        InptMtx(:,:,2) = abs([x(subj,:), dR(subj,:)]');  % R
        ...
        % CÁlCULO DO ITD
        new_itd = itd_synthesis(x(subj, 1), x(subj, 2), out_pos, Fs_sim, 'spheric');
        ...
        if sofa_flag 
            ...
            Sofa = sofaResample(Obj, Fs_out, 4096);
            file_path = ([output_path, FileName, '_', num2str(Fs_out/1000), 'kHz_' num2str(subj) '.sofa']);

            compression = 0;
            SOFAsave(file_path, Sofa, compression);
        end
        ...
        if hesuvi_flag 
            ...
            HeSuViExport(Obj, FileName, output_path, Fs_out, hesuvi_angles, subj)
        end
    end
...
%% INTERNAL FUNCTIONS
function HeSuViExport(Obj_in, FileName, output_path, Fs_out, angles, subj)
    %% Find index of speaker angles
%            [FR,  SR,  RR, FF, FL,  SL,  RL] 
    % azim = [30, 110, 150, 00, 330, 250, 210]; 
    % elev = 0; 
...
%%% Assemble and Processing %%% -------------------------------------------
    ...
    n = 0;
    for k = 1:length(azim)
        [~,pos_idx] = min(sqrt((Obj_in.SourcePosition(:,1) - azim(k)).^2 + (Obj_in.SourcePosition(:,2) - elev(k)).^2));
%         pos_idx = find(Obj_in.SourcePosition(:,1)==azim(k) & Obj_in.SourcePosition(:,2)==elev(k)); %#ok<SAGROW>
        
        clear Obj_out
        Obj_out = SOFAgetConventions('SimpleFreeFieldHRIR'); % dumb way(?)
...
%         Obj_out = sofaAddLowFrequency(Obj_out);
        ...
        IR = squeeze(shiftdim(Obj_out.Data.IR, 2)); 
...
%%% Export to .wav %%%-----------------------------------------------------
    ...
    file_path = ([output_path, FileName, '_', num2str(Fs_out/1000), 'kHz_' num2str(subj) '.wav']);
    audiowrite(file_path, y, Obj_out.Data.SamplingRate, 'BitsPerSample', 64)
end
...
function Obj = sofaResample(Obj, Fs, Nintp)
    ...
    for k = 1:size(Obj.Data.IR, 1)
        for l = 1:size(Obj.Data.IR, 2)
            IRpre(k, l, :) = resample(squeeze(Obj.Data.IR(k, l, :)),p,q,lpFilt);
